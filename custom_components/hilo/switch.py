...
from .const import DOMAIN, SWITCH_CLASSES as HILO_SWITCH_CLASSES
from .hilo_device import HiloBaseEntity


async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    switch_classes = SWITCH_CLASSES + HILO_SWITCH_CLASSES
    entities = []
    if hass.data[DOMAIN].light_as_switch:
        switch_classes.append("LightSwitch")
    if not switch_classes:
        _LOGGER.warning("No switch classes found, skipping")
        return True
