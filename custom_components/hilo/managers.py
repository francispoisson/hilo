...
class UtilityManager:
    ...
    def add_meter_entity(self, entity):
        for tarif in TARIFF_LIST:
            ...
            _LOGGER.debug(f"Creating UtilityMeter entity: {name} {tarif}")
    ...
    def add_meter_config(self, entity):
        name = f"{entity}_{self.period}"
        _LOGGER.debug(f"Creating UtilityMeter config: {name}")
    ...
    async def update(self, hass, config, async_add_entities):
        _LOGGER.debug("Setting up UtilityMeter entities")
        config[UTILITY_DOMAIN] = OrderedDict(
            {**self.meter_configs, **config.get(UTILITY_DOMAIN, {})}
        )
        await utility_setup(hass, config)
        await utility_setup_platform(hass, config, async_add_entities, self.meter_entities)
...
class EnergyManager:
    ...
    def add_flow_from(self, sensor, rate):
        ...
        _LOGGER.debug(f"Adding {sensor} / {rate} to grid source")
    ...
    def add_device(self, sensor):
        ...
        if any(d["stat_consumption"] == sensor for d in self.dev):
            return
        _LOGGER.debug(f"Adding {sensor} to individual device consumption")
        self.updated = True
    ...
    async def update(self):
        if not self.updated:
            return
        _LOGGER.debug("Pushing config to the energy dashboard")
